...
function Counter({ target, label, emoji, suffix = "" }: { target: number; label: string; emoji?: string; suffix?: string }) {
    ...
    return (
        <div
            ref={ref}
            className=" flex justify-center items-center group relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4 transition-all duration-500 hover:-translate-y-2 hover:bg-white/10 hover:border-gold/30 hover:shadow-2xl hover:shadow-gold/5 flex items-center gap-4 md:gap-5"
        >
            {/* ایموجی */}
            ...
            <div className="flex flex-col">
                <div className="text-5xl md:text-3xl lg:text-4xl font-black text-white tracking-tight">
                    {toPersianNumber(count)}
                    <span className="text-gold">{suffix}</span>
                </div>
                <div className="text-xs md:text-sm text-white/50 font-medium tracking-wide text-white">
                    {label}
                </div>
            </div>
        ...
        <section className="relative py-20 md:py-28 overflow-hidden bg-gradient-to-br from-emerald-dark via-emerald-dark/95 to-emerald-medium">
            ...
            <div className="container mx-auto px-4 max-w-6xl relative z-10">
                ...
                <div className="text-center mb-14">
                    ...
                    <p className="text-white mt-5 mb-4 text-base max-w-2xl mx-auto font-light ">
                ...
                <div className="flex flex-wrap justify-center items-stretch gap-4 md:gap-6">
                    {achievements.map((item) => (
                        <div key={item.id} className="flex-1 min-w-[160px] max-w-[280px] p-10">
                            <Counter target={item.target} label={item.label} suffix={item.suffix} />
                        </div>
                    ))}
                </div>

                {/* پایین بخش */}
                <div className="mt-6 flex flex-col sm:flex-row justify-center items-center gap-4 text-white/20 text-xs tracking-widest">
                    <div className="flex items-center gap-3">
                        <span className="w-8 h-px bg-gradient-to-r from-transparent to-gold/30 " />
                        <span className={"text-white"}>✦ افتخار آفرینان ✦</span>
